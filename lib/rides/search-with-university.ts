...
import { SearchRidesRequest, SearchRidesSchema } from './types'
import { RideWithDriver } from './types'
...
export interface RideSearchResult {
  rides: (RideWithDriver & { driverEmail: string })[]
  hasMore: boolean
  total: number
}
...
export async function searchRidesWithUniversity(
  ...
  try {
    ...
    const transformedRides: (RideWithDriver & { driverEmail: string })[] = rides.map(ride => ({
      id: ride.id,
    ...
    // If no user is authenticated, show all rides (open access)
